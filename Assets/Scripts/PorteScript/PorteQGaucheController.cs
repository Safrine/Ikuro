...
public class PorteQGaucheController : MonoBehaviour {
    ...
    //Deplacement
    private float initPos = -12f;
    private float finalPos = 1.57f;
    ...
    void Update()
    {
        ...
        if (collisionPlayer)
        {
            if (finalPos > porteTransform.localPosition.x + 1)
            {
        ...
        else if (porteOuverte)
        {
            ...
            if (waitCount == 90)
            {
                if (initPos <= porteTransform.localPosition.x)
                {
                    porteTransform.localPosition = new Vector3(porteTransform.localPosition.x - 1, porteTransform.localPosition.y, porteTransform.localPosition.z);
                }
                else
